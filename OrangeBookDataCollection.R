...
###############################################################################
# dependencies
sapply(c("httr", "RCurl", "reshape2", "tidyverse"), 
...
# function set to query PubChem Compound DB with Ingredient Name to get their CID
QueryPubChem4CID <- function(x){
  gsub("\n", ";", trimws(
    getURL(paste0(prolog, input, gsub(" ", "%20", x),"/cids/TXT"))))
}
# getting CID attribute value from the PubChem resource
...
IngredientsWithSalt <- lapply(IngredientsWithSalt, 
                              setNames, c("Ingredient", "SaltForm"))
...
IngredientsWithSalt.df <- merge(IngredientsWithSalt.df, 
                                paracelsusDf, by = "Ingredient")

# dataframe with 506 entries by 3 columns (Ingredient name, CID and salt form 
# when applicable)
paracelsusDf <- rbind.fill(
  paracelsusDf[(paracelsusDf$Ingredient %in% OneTokenIngredient),], 
  IngredientsWithSalt.df)
...
paracelsusDf.mesh <- merge(paracelsusDf, meshSet.df[,c(2,3)], 
                           by.x = "Ingredient", by.y = "label")

###################################################################################
tmp <- setdiff(paracelsusDf$Ingredient, paracelsusDf.mesh$Ingredient)
